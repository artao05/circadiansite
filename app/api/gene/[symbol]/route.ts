...
export async function GET(
  ...
  try {
    ...
    if (!data.results || data.results.length === 0) {
      return NextResponse.json(
        { error: "No matching protein found" },
        { status: 404 },
      );
    }
    ...
    if (result.comments) {
      const functionComment = result.comments.find(
        (c: { commentType: string; texts?: { value: string }[] }) =>
          c.commentType === "FUNCTION",
      );
      if (
        functionComment &&
    ...
    try {
      ...
      if (targetId) {
        ...
        const diseases =
          diseaseJson.data?.target?.associatedDiseases?.rows?.map(
            (row: {
    ...
    return NextResponse.json(
      { error: "Failed to fetch live gene data" },
      { status: 500 },
    );
  }
}
