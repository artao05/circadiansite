...
const col = (c, fb) => new T.Color(PAL[c] || fb || '#cccccc');
  ...
  function create(canvas, opts) {
    ...
    function focusNode(id) {
      ...
      else { const n = window.BRAIN && window.BRAIN.nodes ? window.BRAIN.nodes.find(x => x.id === id) : null; if (n) focusCategory(n.category); }
    }
    ...
    const ro = new ResizeObserver(() => requestAnimationFrame(resize)); ro.observe(dom.parentElement || dom);
    resize();
    ...
    return {
      ...
      addGlowingSphere(id, position, colorHex, size = 0.005) {
        ...
        mesh.userData = { 
          ...
          nodeId: id, 
          cat: 'other', 
          baseColor: new T.Color(colorHex), 
          baseEmiss: 2.0 
        };
        model.add(mesh);
        allMeshes.push(mesh);
  ...
  export const BrainScene = { create };
