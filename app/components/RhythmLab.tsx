...
export function RhythmLab() {
  ...
  if (syncedMasterHour !== masterHour) {
    ...
    const isExternalChange = Math.round(state.phase) % 24 !== masterHour;
    if (isExternalChange) {
      const currentDayOffset = Math.floor(state.phase / 24) * 24;
      let newPhase = currentDayOffset + masterHour;
      if (newPhase > 48) newPhase = masterHour;
      setState((current) => ({ ...current, phase: newPhase }));
    }
  }
  ...
  const chart = useMemo(() => {
    ...
    const cortisolBase = [
      25, 25, 28, 35, 45, 60, 75, 85, 80, 70, 60, 55, 50, 45, 40, 37, 34, 30,
      28, 26, 25, 24, 24, 24, 25,
    ];
    const cbtBase = [
    ...
    const generateCurve = (
      ...
      return d3.range(0, 49, 0.5).map((t) => {
        const normTime =
          ((t % effectiveState.period) / effectiveState.period) * 24;
    ...
    const referencePoints = d3.range(0, 49, 0.5).map((t) => {
      const rhythm =
        50 +
        effectiveState.amplitude *
          Math.sin(((t - 6) / effectiveState.period) * Math.PI * 2);
      return { t, value: Math.max(0, Math.min(104, rhythm)) };
    });
    ...
    const normPhase =
      ((phaseT % effectiveState.period) / effectiveState.period) * 24;
      ...
      refY: y(
        Math.max(
          ...
          Math.min(
            ...
            50 +
              effectiveState.amplitude *
                Math.sin(((phaseT - 6) / effectiveState.period) * Math.PI * 2),
          ),
        ),
      ),
    };
    ...
    <div className="interactive-block rhythm-lab">
      ...
      >
        ...
        <svg viewBox={`0 0 ${chart.width} ${chart.height}`} role="img">
          ...
          {/* Y-Axes */}
          <g className="y-axes" style={{ fontSize: "10px", fill: "var(--muted)" }}>
            {/* Cortisol Axis */}
            <line
              x1={chart.width - 100}
              x2={chart.width - 100}
      ...
      <div className="control-panel" aria-label="Rhythm controls">
        ...
        <p className="microcopy">
          Use the Current Time slider to move the vertical marker across the
          48-hour period and sample the expected physiological levels at any
          given point.
        </p>
      </div>
    </div>
